...
gcloud run jobs deploy bestofbot-animals --project=$project --region=$region --source . --set-env-vars=BOTUSER="best_of_animals_bot",BOTINSTANCE="lemmy.dbzer0.com",COMMUNITY="$comm",COMMLIST="configs/_animals_comms.json",MODUSER="$mod_user",IMAGES_ONLY=0,NSFW_BEHAVIOUR=2,GIST=$gist,POSTTITLE="❗Top Animal posts of the week❗" --set-secrets="BOTPW=best_of_wallpaper_bot:latest,MODPW=$mod_pw,GHTOKEN=$ghtoken,TLDRTHIS_API=tldrthis,SMMRY_API=smmry" &
